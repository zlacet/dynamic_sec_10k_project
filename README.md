...
This tool answers natural-language questions about any publicly traded company's most recent SEC 10-K annual report. Ask about any company by name, and the system detects the ticker, fetches and indexes its latest 10-K directly from SEC EDGAR on the fly, then answers using only content it finds in that filing.
